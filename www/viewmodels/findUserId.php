...
/********************************************************
    아이디 찾기 KMC 본인인증 return 값 복호화 페이지 
********************************************************/
            ...
            // 01.인증결과 1차 복호화
			//$rec_cert  = exec("$ICERTSecu SEED 2 0 $rec_cert ");
...
$value ="M";
$result = user_check($name , "",$phoneNo,$value);
if ($result[0] == 1){
?>

<form name="result_form" id="result_form" method="post" action="../views/findMemberId.php" >
<input type="hidden"  id="member_id" name="member_id"  value=<? echo $result[1]; ?>> 
<input type="hidden"  id="join_date" name="join_date"  value=<? echo $result[2]; ?>> 
</form>
<script>
document.result_form.target = opener.window.name;
...
<form name="result_form" id="result_form" method="post" action="../views/findMemberId.php" >
<input type="hidden"  id="not_member" name="not_member"  value="true"/> 
</form>
<script>
document.result_form.target = opener.window.name;
document.getElementById('result_form').submit();
