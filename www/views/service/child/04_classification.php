    ...
    <div class="content" style="background-color:#fff;height:713px;">
        ...
        <form id='service_info_form' name="service_info_form" action='service_info.php' method='post'>
            <div class="classification_inner">
                ...
                <div style="float:left; vertical-align:top; margin:-20px 40px 0px 0px; border: 1px solid #d2d2d2; padding: 15px 20px 88px 20px; border-radius: 7px;">
                    ...
                    <div style="float:left; vertical-align:top; margin:-20px 40px 0px 0px; border: 1px solid #d2d2d2; padding: 15px 20px 125px 20px; border-radius: 7px;">
                        <div style="margin-bottom:10px">
                    ...
                    <div style="float:right; vertical-align:top;margin:-20px 0px 0px 0px; border: 0px solid #d2d2d2; padding: 15px 0px 0px 20px; border-radius: 7px;">
                        <div style="float:right; vertical-align:top; margin-right: 10px; padding: 0px;">
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Narrow_arch" <? if ($_SESSION['narrow_arch'] == "on") echo checked; ?>
                                       name="narrow_arch" />
                                <label for="Narrow_arch">Narrow arch</label>
                            </div>
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Prolapse_of_anterior_teeth" <? if ($_SESSION['anterior_teeth'] == "on") echo checked; ?>
                                       name="anterior_teeth" />
                                <label for="Prolapse_of_anterior_teeth">Prolapse of anterior teeth</label>
                            </div>
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Improve_smile_line" <? if ($_SESSION['smile_line'] == "on") echo checked; ?>
                                       name="smile_line" />
                                <label for="Improve_smile_line">Improve smile line</label>
                            </div>
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Inclined_occlusal_plane" <? if ($_SESSION['inclined_occlusal_plane'] == "on") echo checked; ?>
                                       name="inclined_occlusal_plane" />
                                <label for="Inclined_occlusal_plane">Inclined occlusal plane</label>
                            </div>
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Abnormal_tooth_shape" <? if ($_SESSION['abnormal_teeth_shape'] == "on") echo checked; ?>
                                       name="abnormal_teeth_shape" />
                        ...
                        <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Crowding" <? if ($_SESSION['crowding'] == "on") echo checked; ?>
                                       name="crowding" />
                                <label for="Crowding">Crowding</label>
                            </div>
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Diastemata" <? if ($_SESSION['diastemata'] == "on") echo checked; ?>
                                       name="diastemata" />
                                <label for="Diastemata">Diastemata</label>
                            </div>
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Asymmetric" <? if ($_SESSION['asymmetric'] == "on") echo checked; ?>
                                       name="asymmetric" />
                                <label for="Asymmetric">Asymmetric</label>
                            </div>
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Vertical_open_bite" <? if ($_SESSION['vertical_open_bite'] == "on") echo checked; ?>
                                       name="vertical_open_bite" />
                                <label for="Vertical_open_bite">Vertical open bite</label>
                            </div>
                            <div style="margin-bottom:10px">
                                <input type="checkbox"
                                       id="Horizontal_open_bite" <? if ($_SESSION['horizontal_open_bite'] == "on") echo checked; ?>
                                       name="horizontal_open_bite" />
                                <label for="Horizontal_open_bite">Horizontal open bite (Overjet)</label>
                            </div>
                            <div style="margin-bottom:10px">

                                <input type="checkbox" id="Etc" <? if ($_SESSION['etc'] == "on") echo checked; ?>
                                       name="etc" />
                                <label for="Etc">Etc</label>

                                <div class="form-group" style="margin:-20px 0px 0px 80px; width:160%; ">
                                    <textarea
                                            class="form-control-sign_up" <? if ($_SESSION['etc'] != "on") echo disabled; ?> name="etc_value"
                                            id="etc_value" placeholder="Enter your etc."
                                            style="height:70px;"><?= $_SESSION['etc_value'] ?></textarea>
